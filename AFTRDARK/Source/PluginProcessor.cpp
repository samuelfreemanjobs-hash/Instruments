...
namespace
{
using PID = aftrdark::ParameterIDs;
...
juce::AudioProcessorValueTreeState::ParameterLayout AftrdarkAudioProcessor::createParameterLayout()
{
...
AftrdarkAudioProcessor::AftrdarkAudioProcessor()
#ifndef JucePlugin_PreferredChannelConfigurations
    : AudioProcessor (BusesProperties().withOutput ("Output", juce::AudioChannelSet::stereo(), true)),
#endif
      apvts_ (*this, nullptr, "AFTRDARK", createParameterLayout())
{
    applyFactoryPreset (0);
}

AftrdarkAudioProcessor::~AftrdarkAudioProcessor() = default;

const juce::String AftrdarkAudioProcessor::getName() const
{
    return JucePlugin_Name;
}

bool AftrdarkAudioProcessor::acceptsMidi() const { return true; }
bool AftrdarkAudioProcessor::producesMidi() const { return false; }
bool AftrdarkAudioProcessor::isMidiEffect() const { return false; }
double AftrdarkAudioProcessor::getTailLengthSeconds() const { return 2.0; }
bool AftrdarkAudioProcessor::hasEditor() const { return true; }

int AftrdarkAudioProcessor::getNumPrograms()
{
    return static_cast<int> (aftrdark::presets::getFactoryPresets().size());
}

int AftrdarkAudioProcessor::getCurrentProgram() { return currentProgram_; }

void AftrdarkAudioProcessor::setCurrentProgram (int index)
{
    applyFactoryPreset (index);
}

const juce::String AftrdarkAudioProcessor::getProgramName (int index)
{
    const auto& presets = aftrdark::presets::getFactoryPresets();
    if (index >= 0 && index < static_cast<int> (presets.size()))
        return juce::String (presets[static_cast<std::size_t> (index)].name.data());
    return {};
}

void AftrdarkAudioProcessor::changeProgramName (int index, const juce::String& newName)
{
    juce::ignoreUnused (index, newName);
}

void AftrdarkAudioProcessor::applyFactoryPreset (int index)
{
    const auto& presets = aftrdark::presets::getFactoryPresets();
    if (index < 0 || index >= static_cast<int> (presets.size()))
        return;
...
void AftrdarkAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void AftrdarkAudioProcessor::releaseResources()
{
    engine_.reset();
}

bool AftrdarkAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    const auto& main = layouts.getMainOutputChannelSet();
    return main == juce::AudioChannelSet::mono() || main == juce::AudioChannelSet::stereo();
}

aftrdark::SynthParams AftrdarkAudioProcessor::readParamsFromApvts() const noexcept
{
...
void AftrdarkAudioProcessor::refreshParameters() noexcept
{
    liveParams_ = readParamsFromApvts();
}

void AftrdarkAudioProcessor::pushParamsToEngine() noexcept
{
    engine_.setParams (liveParams_);
}

void AftrdarkAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
...
juce::AudioProcessorEditor* AftrdarkAudioProcessor::createEditor()
{
    return new AftrdarkAudioProcessorEditor (*this);
}

void AftrdarkAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void AftrdarkAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
{
    return new AftrdarkAudioProcessor();
}
