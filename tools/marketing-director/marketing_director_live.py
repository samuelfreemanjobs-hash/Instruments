#!/usr/bin/env python3
"""
Marketing Director — multi-agent orchestrator (LOCAL DEV ONLY).

Do not import from CI or business_agent.py. Requires ANTHROPIC_API_KEY.

Architecture: Hierarchical / Supervisory — specialists as tools with nested API loops.
"""
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "audience": {"type": "string"},
                "channels_available": {"type": "array", "items": {"type": "string"}},
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "regulated_category": {"type": "string"},
            },
...
class CopyAgent(BaseSpecialist):
    SYSTEM_PROMPT = f"You are Copy specialist for {COMPANY}. Return JSON with variants."


class ComplianceAgent(BaseSpecialist):
    SYSTEM_PROMPT = f"""\
You are Compliance for {COMPANY}. Return JSON: severity (NONE|LOW|MEDIUM|HIGH|CRITICAL), \
issues[], required_disclaimers[], ready_to_publish bool. ready_to_publish false if severity >= MEDIUM.
"""
...
class MarketingDirector:
    def __init__(
        ...
        self.specialists: dict[str, BaseSpecialist] = {
            "copy_agent": CopyAgent(
                ...
                self.models.copy,
                self.budgets.copy,
                tool_executor=self._specialist_tool_executor,
                trace_id=self.trace_id,
            ),
            "compliance_agent": ComplianceAgent(
                client,
                self.models.compliance,
                self.budgets.compliance,
                trace_id=self.trace_id,
            ),
        }
    ...
    def _dispatch(self, tool_name: str, tool_input: dict) -> dict:
        if tool_name in self.specialists:
            ...
            return {"status": result.status, "output": result.output, "notes": result.notes}
        ...
        return {"error": f"unknown tool: {tool_name} (wire more specialists in live mode)"}
    ...
    def _force_escalate(self, tool_name: str) -> dict:
        return self.human_review_handler(
            {
                "reason": f"Repeated calls to {tool_name}",
                "urgency": "elevated",
