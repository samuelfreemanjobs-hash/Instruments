...
void {{PLUGIN_CAMEL}}AudioProcessor::updateSmootherTargets() noexcept
{
...
float {{PLUGIN_CAMEL}}AudioProcessor::tubeSaturation(float sample, float driveAmount) noexcept
{
    const float gain = 1.0f + driveAmount * 24.0f;
    return std::tanh(sample * gain);
}

void {{PLUGIN_CAMEL}}AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
