...
- **Parameter lookups**: cache `apvts.getRawParameterValue(...)` in the constructor (see `plugin-factory/plugins/<your-plugin>/PluginProcessor.cpp`). Never call `getRawParameterValue("name")` inside `processBlock`.

This repo’s reference implementation: `plugin-factory/plugins/<your-plugin>/` (effect demo matching the example feature list).
