#!/usr/bin/env python3
"""Create disklordz/antigravity/inbox handoff JSON from an Airtable Agent Work Order."""
...
BRIDGE = REPO_ROOT / "scripts" / "antigravity-bridge" / "antigravity-bridge.sh"
...
def main() -> None:
    ...
    parser.add_argument("--patch-airtable", action="store_true", help="Set antigravity_handoff_id on WO if field exists")
    ...
    if "[Plugin][HISE]" not in title and "antigravity" not in owner.lower():
        raise SystemExit(
            f"WO {args.work_order_id} is not Antigravity lane "
            f"(title must include [Plugin][HISE] or owner_agent antigravity-hise)"
        )
    ...
    cmd = [
        ...
        "--path",
        "docs/HISE_ANTIGRAVITY_LANE.md",
        "--path",
        "disklordz/antigravity/AGENT_INSTRUCTIONS.md",
    ]
    ...
    if args.patch_airtable and hid:
        try:
            patch_wo(rec["id"], {"antigravity_handoff_id": hid, "status": "In Progress"})
