...
export function resolveDrumParams(prompt: string, presetId: string): DrumParams {
  const base = PRESET_BASE[presetId] ?? PRESET_BASE["mpc-ready-808"];
  const seed = hashSeed(prompt, presetId);
  const jitter = (i: number) => ((seed >> (i * 4)) & 0xf) / 0xf - 0.5;
  ...
  return {
    ...
    clapWide: clamp((base.clapWide ?? 0.4) * (1 + jitter(7) * 0.1), 0.15, 0.65),
    seed,
  };
}
