#pragma once
/* PORT_COMPLETE — TR-909 kit: kick/snare/CH/OH via MIDI map (36/38/42/46). */
#include "processor.h"
#include "unit_osc.h"
...
public:
  ...
  void reset() override final {
    ...
    kick_decay_ = 0.62f;
    snare_snap_ = param_10bit_to_f32(737);
    ch_decay_ = 0.028f;
    oh_decay_ = 0.14f;
...
private:
  ...
  void triggerKick(float vel) {
    kick_.env = vel * (0.85f + punch_ * 0.25f);
    kick_.phase = 0.f;
    kick_.pitch_mul = 2.4f + punch_ * 3.2f;
    kick_.w0 = 0.0042f * tune_mul_;
    kick_.lp = 0.f;
  }

  void triggerSnare(float vel) {
    snare_.env = vel * (0.7f + snare_snap_ * 0.35f);
    snare_.tone_phase = 0.f;
    snare_.tone_w = 0.015f * tune_mul_;
    snare_.noise_lp = 0.f;
  }
  ...
  float processKick(float decay_coef, float fs) {
    ...
    kick_.pitch_mul += (1.f - kick_.pitch_mul) * (1.f / (kick_decay_ * fs * 0.11f));
    kick_.phase += kick_.w0 * kick_.pitch_mul;
    if (kick_.phase >= 1.f) {
      kick_.phase -= floorf(kick_.phase);
    }
    float s = sinf(6.28318530718f * kick_.phase);
    s += (2.f * kick_.phase - 1.f) * (0.25f + tone_ * 0.2f);
    kick_.env *= decay_coef;
    const float g = 1.f - expf(-6.28318530718f * (55.f + tone_ * 120.f) / fs);
  ...
  float processSnare(float fs) {
    ...
    const float body = sinf(6.28318530718f * snare_.tone_phase) * (0.42f + snare_snap_ * 0.28f);
    float n = noise();
    const float g = 1.f - expf(-6.28318530718f * (1200.f + tone_ * 5500.f) / fs);
    snare_.noise_lp += g * (n - snare_.noise_lp);
    snare_.env *= expf(-6.f / ((0.05f + snare_snap_ * 0.1f) * fs));
  ...
  float processHat(HatVoice &h, float fs) {
    ...
    float n = noise();
    const float g = 1.f - expf(-6.28318530718f * (7000.f + tone_ * 9000.f) / fs);
    h.lp += g * (n - h.lp);
    const float d = (&h == &ch_) ? ch_decay_ : oh_decay_;
    h.env *= expf(-6.f / (d * fs));
    return h.lp * h.env * 0.62f;
  }
