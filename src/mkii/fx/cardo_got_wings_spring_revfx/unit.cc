...
static CardoSpringRevfx s_processor_instance;
static int32_t cached_values[UNIT_REVFX_MAX_PARAM_COUNT];
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc)
{
  ...
  if (s_processor_instance.getBufferSize() > 0) {
    float *buf = (float *)desc->hooks.sdram_alloc(s_processor_instance.getBufferSize() * sizeof(float));
    if (!buf)
      return k_unit_err_memory;
    std::fill(buf, buf + s_processor_instance.getBufferSize(), 0.f);
    s_processor_instance.init(buf);
  } else {
